...
//   }
  ...
  /**
	 ...
	 * @param obj
	 *          article_data | post
	 *          对象{'msgbody'=>"回复内容",'postID'=>144819,"typeID"=>1,}
	 */
  async reply_add(obj){
...
// }
  ...
  async uploadFile(file, type){
    ...
    return await this.sendpost("article/user/upload_file/?type="+type, data)
  }
...
/**
 * //导出工具类 export default Blog;
 * 
 * //使用vuex进行状态管理的方式调用, 适用于需要在多组件间共享数据的情况
 * 
 * //导入 import Blog from '../../../../service/blog' let blogModule ={ state: {
 * object: new Blog() }, mutations: { increment (state) { state.count++ } },
 * actions: {
 *  } }
 * 
 * ... , beforeCreate(){//注册 this.$store.registerModule('blog', blogModule) },
 * methods:{ test(){//使用 console.log('hello edit vue');
