...
include DOCUROOT.'/article/Action/baseAction/_content.php';

class composeContent extends contentAction{
	protected $editorCate='pageContent';//用于编辑器内文件上传时，记录上传文件所用的数据库标识
	
	function __construct() {
		parent :: __construct();
		$this->mod = load("article_content");
		$this->isshow=true;
		
		//初始化文章级别类
		if(!in_array($_GET['act'],array('del','order'))) {
			//索引页导航参数
			$this->menuStr='admin.page.incmenu.setup_composeContent';
			//当前项目配置
