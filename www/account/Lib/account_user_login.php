...
class account_user_login extends Model{
	...
	public function wxc_login($login_data,$login_token){
	    ...
	    $check_account_user=$obj_account_user->getOne("*",['login_data'=>$login_data]);
	    ...
	    //设置登录cookie
	    $check_account_user_login=$this->getOne(["id","pointer"],["userID"=>$check_account_user['id']]);
	    $rand=md5(times::gettime().$this->rand());
	    if(empty($check_account_user_login)){
	        $index="index0";
	        $this->insert([$index=>$rand,"userID"=>$check_account_user['id']]);
	    }else{
	        $index="index{$check_account_user_login['pointer']}";
	        $point=substr($check_account_user_login['pointer']+1,-1);
	        $this->update([$index=>$rand,"pointer"=>$point],["userID"=>$check_account_user['id']]);
	    }
	    
	    $cookie=$this->encrypt($rand."___".$check_account_user['id']."___".$index);
