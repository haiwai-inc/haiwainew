...
class page{
	...
	public function GetList($sql,$maxRows_Rs=25,$DHpagenum=5)
	{
		...
		$this->total=$this->_getRecordCountBySQL($sql);
	...
	public function readList($fields, $where, $table, $maxRows_Rs=25,$DHpagenum=5){
		...
		if($this->dbtype =='MySQL'){
			...
			if(!is_array($fields) && trim($fields)!="*"){
				//兼容SQL语句
				$this->total = $this->_getRecordCountBySQL( dbtools::getSQL($fields,null,null) );
				$this->result = $this->conn->getAll($fields);
			}else{
				$this->total = $this->_getRecordCount( $where, $table );
	...
	static function getIDs($type="I",$name='id',$intval=true){
		if(!empty($_GET[$name])) return ($intval)?intval($_GET[$name]):$_GET[$name];
	 ...
	 * @return int
	 */
	private function _getRecordCountBySQL($sql){
		$sqlstr = $sql;
		$pos = stripos($sqlstr, 'order by');
		if(!empty($pos)) $sql = substr($sql,0,$pos);
		
		$pos = strpos($sqlstr, 'LIMIT');
		if(!empty($pos)) $sql = substr($sql,0,$pos);
		
		$start = stripos( $sqlstr, "select") + 6;
		$end = stripos( $sqlstr, "from ");
	...
	private function _getRecordCount( $where, $table ){
		$tbpk = empty($this->tablePK)?"*":$this->tablePK;//表主键
		$fields = array("SQL"=>"count({$tbpk})as `num`");
		
		foreach($where as $k=>$v){
			$field = strtolower($k);
			if(in_array($field,array('order','limit'))) unset($where[$k]);
		}
		
		$rs = $this->conn->getOne( $fields, $where, $table );
		$num = empty($rs)?0:$rs['num'];
