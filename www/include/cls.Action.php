...
abstract class Action {
	...
	public function isAMP(){
		...
		if(!strpos($_SERVER['HTTP_REFERER'],"www.google.com"))return false;
		if(http::isMobile() && !http::isTablet()) return true;
		return false;
	}
	...
	function setCache($cacheID, $cache, $cacheTime=0){
		...
		foreach($cachePools as $host)
			if( $obj=func_initMemcached($host) ) $obj->set($cacheID, $cache, $cacheTime);
	}
	...
	// 检查当前内容是否在允许的域名下
	public function checkdomain($allowDomainList){
		$domain = $_SERVER['HTTP_HOST'];
		if(in_array( $domain, $allowDomainList)) return;
		if(isset($_GET['skipDomainCheck'])) return;
		
		$prefix = substr($domain,0,5);
		
		//测试
		if($prefix=='beta1') return;
		if($prefix=='node7') return;
		if($prefix=='node8') return;
		if($prefix=='node9') return;
		if($prefix=='web1.') return;
                if($prefix=='web2.') return;
                if($prefix=='web3.') return;
                if($prefix=='web4.') return;
		if($prefix=='web5.') return;		


		if($prefix=='beta.') return;
		if($prefix=='test.') return;
		if($prefix=='zhbet') return;//繁体测试环境
		if($prefix=='zh.we') return;//繁体生产环境
		if($prefix=='admin') return;
		if($prefix=='local') return;
		
		$scheme = $_SERVER['REQUEST_SCHEME']=='https'?'https':'http';
		$host = $allowDomainList[0];
	...
	public function display(){
		...
		$lang = substr($host,0,2);
		
		//简转繁
		if($lang=='zh'){
			$obj = new big5();
			$html = $obj->c2t($this->fetch());
		}
		
		//繁转简
		/*if($lang=='cn'){
			$obj = new big5();
			$html = $obj->t2c($this->fetch());
		}*/
	...
	private function getContentType(){
		...
		$ext = strtolower($ext);
		if(in_array($ext,array('php','html','htm'))) header("cache-control:max-age=60");
		if($ext=="php") return;
