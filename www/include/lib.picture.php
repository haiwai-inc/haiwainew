...
class picture{
    ...
    static $http_cache; //用于存储远程抓取的缓存数据
    ...
    function GetSize( $type ){
        list($w, $h, $extnum) = self::getImageSize($this->filepath);
        $this->width_orig = $w;
        $this->height_orig = $h;
        $this->extnum = $extnum;
    ...
    function readimage () {
        ...
        //处理本地图片
        if( substr(strtolower($this->filepath),0,4)!='http'){
            switch ($this->extnum) {
                case 1:
            ...
            $http_fetch_data = self::curl_fetch( $this->filepath );
            $image = imagecreatefromstring( $http_fetch_data['data'] );
        }
    ...
    function imgwater($img, $position='RB', $waterimg='', $quality=75 ){
        ...
        $imginfo = self::getImageSize($img);		//获取图片信息
        $wimg = self::getImageSize($waterimg);	//获取水印图片信息
    ...
    static function getImageInfo($filename, $referer = ''){
        $IMAGETYPE= self::$IMAGETYPE;
        if(!list($w, $h,$extnum) = self::getImageSize( $filename )) return false;
    ...
    static function getImageSize( $filename, $referer = ''){
        //本地图片
        if( substr(strtolower($filename),0,4)!='http') return getimagesize( $filename );
        
        //处理远程图片
        $http_fetch_data = self::curl_fetch( $filename, $referer);
        
        $http_info = $http_fetch_data['http_info'];
        $data = $http_fetch_data['data'];
        $curl_errno = $http_fetch_data['curl_errno'];
        
        // Get network stauts
        $http_status = $http_info['http_code'];
        
        // Check network stauts
        if ( $http_status != 200 ) {
    ...
    static function curl_fetch($url, $referer = ''){
        $url = trim($url);
        $cacheID = md5($url);
        
        if( empty(self::$http_cache[$cacheID]) ){
            // Set headers
            ...
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10); //timeout in seconds for network connection
            curl_setopt($ch, CURLOPT_TIMEOUT, 30); //timeout in seconds for getting data
            ...
            self::$http_cache[$cacheID] =  [ 'data'=>$data, 'http_info'=>$http_info, 'curl_errno'=>$curl_errno];
        }
        
        return self::$http_cache[$cacheID];
    }
    
    static function getPicTrueType($path,$info=false){
        if(!file_exists($path)) return false;
        list($w,$h,$extnum) = self::getImageSize($path);
    ...
    static function QRCode($data,$widhtHeight='150',$EC_level='L',$margin='0'){
        ...
        return "https://api.qrserver.com/v1/create-qr-code/?size={$widhtHeight}x{$widhtHeight}&data=".$url;
    }
}
?>