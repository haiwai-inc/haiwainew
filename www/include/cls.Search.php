...
class Search {
	...
	function __construct(){
		$this->search_host = defined('SEARCH_NODE_HOST')?SEARCH_NODE_HOST:'http://127.0.0.1';
	...
	function curl($url, $data=null, $method="GET", $bulk=null){
		...
		if(!empty($data)) {
			if(!empty($bulk)){
				...
				curl_setopt( $ci, CURLOPT_HTTPHEADER, array('Content-Type: application/x-ndjson') );
	...
	private function __getNavData(){
		...
		if ( $numAll > 0 && $pageNow<=$pageAll ){
			...
			$list["first"] = $start + 1;
			$list["last"] = min($start+$pageSize, $numAll);
	...
	private function __getNavTpl(){
		if(!empty($this->pageNavTpl)){
			...
			$navtpl = DOCUROOT.'/include/template/searchnav/cn.html';
			if(!is_file($navtpl)){
				$navtpl = DOCUROOT.'/include/template/searchnav/cn.html';
			}
		}
	...
	private function __getLink($pn){
		...
		if(!isset($url)){
			...
			if( isset($_GET[$this->page['param']]) ) unset($_GET[$this->page['param']]);
	...
	function add($data,$id=''){
		...
		return $this->call( "/{$this->index}/_doc/{$id}", $data, $method );
	}
	// add function to update multiple documents at the same time in order to boost performance while initializing index
	//TODO check for existing entries or delete before adding
	function addBulk($data){
		$method = empty($id)?'POST':'PUT';
		return $this->call( "/{$this->index}/_bulk", $data, $method, 1 );
	}
	
	function update($data,$id){
		$doc = array("doc"=>$data);
		return $this->call( "/{$this->index}/_update/{$id}", $doc, "POST" );
	}
	
	function del($id){
		return $this->call( "/{$this->index}/_doc/{$id}", array(), 'DELETE' );
	}
	...
	function indexTypedel(){
		return $this->call( "/{$this->index}/{$this->type}/", array(), 'DELETE' );
	}
