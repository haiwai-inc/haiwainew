...
class strings {
	 ...
	 */
	public static function sortArray($data, $KeyName1, $SortOrder1 = "SORT_ASC", $SortType1 = "SORT_REGULAR"){
		...
		$SortData = [];
		// Get keys to sort by and put them to SortData array.
		for($i = 1; $i<$ArgCount; $i++ ){
			$Arg = func_get_arg($i);
			if(substr($Arg,0,5)!="SORT_"){
			    $SortData[] = array_column($data, $Arg);
			}else{
			    $SortData[] = isset($Rule[$Arg])?$Rule[$Arg]:SORT_ASC;
			}
		}
        ...
        //array_multisort($SortData[0],$SortData[1],$SortData[2],$SortData[3],$data);
	...
	public static function remove_script($rs,$level=1,$type="string",$keyname="title"){
		if(empty($rs)){
			return $rs;
		}
		
		if($type=="string"){
			$rs=@self::remove_script_level($rs,$level);
		}
		
		if($type=="array"){
			foreach($rs as $k=>$v){
				$rs[$k][$keyname]=self::remove_script($v[$keyname],$level);
			}
		}
	...
	private function remove_script_level($rs,$level=1){
		
		//轻级别
		if($level==1){
			$rs=preg_replace("/<style\\b[^>]*>(.*?)<\\/style>/is", "", $rs);
			$rs=preg_replace("/<script\\b[^>]*>(.*?)<\\/script>/is", "", $rs);
			
			$rs = strip_tags($rs,'<br><p><a><img>');
		}
