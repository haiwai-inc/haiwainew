...
class editor{
	...
	static function load($config,$content){
		...
		if(!empty($config['editor'])){
			$html= self::load_fck($config,$content);
		}
		else{
			$html= self::load_ckeditor($config,$content);
		}
		*/
	...
	/**
	*ie6,7,8 一定是旧的
	*admin.wenxuecity.com 一定是旧的
	*chrome一定是新的
	*其它使用cookie可选, 默认新的
	*setcookie($key,$lan,time()+31536000,"/",".wenxuecity.com");
	*/	
	...
	{		
		...
		/*
		if(
			preg_match('/(?i)msie [4-8]/',$_SERVER['HTTP_USER_AGENT']) || 
			preg_match('/(?i)admin\.wenxuecity\.com/', $_SERVER['HTTP_HOST']) || 
			preg_match('/(?i)admin_article\.php/', $_SERVER['PHP_SELF'])
		)
		{
			$editor = "old";
		}
	...
	static function load_fck($config,$content){
	...
	static function load_ckeditor($config,$content){
		//新闻编辑特殊editor
		/*
		if(!empty($config['editor'])){
			$html = "<script src='/include/editor/ckeditor/news_ckeditor.js'></script>\n";
		}else{
			$html = "<script src='/include/editor/ckeditor/ckeditor.js'></script>\n";
		}
		*/
		
		$html = "<script src='/include/editor/ckeditor/ckeditor.js'></script>\n";
		
		$html .= "<script src='/include/editor/ckeditor/wxccust/setmode.js'></script>\n";
		$html .='<input  id="configid" type="hidden" value="' .$config["id"] .'" readonly />'; //新添加用于在plugin中获取$config["id"]，感觉好像在ckeditor没必要用config["id"]?
		$html.='<input  id="'. $config["id"] . '___userID" type="hidden" value="'.$config['userID'].'" readonly />';
		$html.='<input  id="'. $config["id"] . '___dataID" type="hidden" value="'.$config['dataID'].'" readonly />';
		$html.='<input  id="'. $config["id"] . '___dataType" type="hidden" value="'.$config['dataType'].'" readonly />';
		$html.='<input  id="'. $config["id"] . '___TempID" type="hidden" value="" readonly/>'; //这个目前好像在upload部分也没有用，暂时保留
		
		//id=editor1, id 用于获取此editor （CKEDITOR.instances.editor1.。。。）
		$html.='<textarea name="' .$config["id"] .'" id="editor1" rows="20" cols="100">' .$content .'</textarea>'; //加载新的ckeditor

		
		//set config，否则用默认的ckeditor/config.js, 可以在wxccust下定义bbs blog 特有的config，目前只做了一个 wxcconfig.js
		$html.='<script type="text/javascript">';
		$html.='CKEDITOR.replace( "editor1", {
			customConfig: "/include/editor/ckeditor/wxccust/wxcconfig.js",
			width:"'.$config['width'] .'", height:"' .$config['height'] .'"
		});';
		
		$html .="CKEDITOR.on('instanceReady', function(){ 
					editor.OnComplete(CKEDITOR.instances.editor1); 
					$('.cke_button__dlgpicture .cke_button_label').css('display','inline');
					$('.cke_button__dlgvideo .cke_button_label').css('display','inline');
					$('.cke_button__dlgmp3 .cke_button_label').css('display','inline');
					$('.cke_button__smiley .cke_button_label').css('display','inline');
