...
var pictureCore ={
	...
	showDemo:function(userID,NickName,tmpID,w,h,fileID,filePathInfo){
		...
		var configid = doc.document.getElementById("configid").value;
		//var tmpIDName = doc.editor.Instance.Name+'___TempID';
		var tmpIDName = configid+'___TempID';
		...
		//$("#confirm").css('display','block'); //不再用此确认按钮，用dialog自带的确认，onOk 定义在plugin.js 中
	...
	setHtml:function(){
		...
		doc.CKEDITOR.currentInstance.insertHtml(html);
		
		//关闭dialog
		if(this.type!="panel") {		
			//parent.Cancel();
			doc.CKEDITOR.dialog.getCurrent().hide();
		}
	},
	...
	cancel:function(){
		...
		if(this.type=='panel'){
			...
			doc.CKEDITOR.currentInstance.Focus();
			//parent.editor.Instance.Events.FireEvent( 'OnSelectionChange' ) ;
			doc.CKEDITOR.currentInstance.Events.FireEvent( 'OnSelectionChange' ) ;
		}
	},
	...
	addfile:function(){
		...
		formObj.action= "/album/upload.php?type=album";
		formObj.submit();
	},
	...
	GetConf:function(id){
		...
		//return doc.document.getElementById("editor1");
		return doc.document.getElementById(configid+'___'+id).value;
	}
...
var videoCore ={
		...
		getEmbedCode:function(src){
			...
			if(((src.indexOf('http://www.youtube.com/')!=-1)||(src.indexOf('https://www.youtube.com/')!=-1)) && (src.indexOf('?v=')!=-1 || src.indexOf('&v=')!=-1)){
				...
				var tmp = "https://www.youtube.com/embed/"; 
				if(src.indexOf('https://www.youtube.com/')!=-1){
					tmp = "https://www.youtube.com/embed/"; 
				}
				url = tmp+token+"?rel=0";
			}
...
var mediaCore ={
	...
	showDemo:function(userID,NickName,tmpID,fileID,filePathInfo){
		...
		$("#music").val("");
		$("#select").css('display','block');
		//$("#confirm").css('display','block');// not using the 确定 button
		
		this.g('music-ifr').src='/include/editor/api/script/audio_ckeditor.php?&url='+filePathInfo;
	},
	...
	cancel:function(){
		...
		$("#select").css('display','block');
		$("#confirm").css('display','none');
		this.g('music-ifr').src='/include/editor/api/script/audio_ckeditor.php';
	...
	addfile:function(){
		...
		formObj.action="/album/upload.php?type=media";
