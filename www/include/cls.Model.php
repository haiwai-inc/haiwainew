...
abstract class Model{
	...
	*/
	...
	function setOrder($id,$order="order", $table=null){
		$this->syncTableInfo();
		$this->checkDbType('MySQL');
		return $this->Update( array("SQL"=>"`{$order}`=`id`"), array('id'=>$id), $this->getTbName($table) );
	}
	...
	function resetAutoid($id=0,$host=null,$port=6379){
		...
		$cacheid = "model-{$this->dbinfo['config']}-{$this->tableName}-autoid";
		$cacheStatus = $rdb->set($cacheid,$id);
		$result = empty($cacheStatus)?0:$id;
	 ...
	 * @return array
	 */
	protected function parseFields($condition){
		//过滤mongo的数据结构
		...
		//针对多行插入的情况 
		if(!empty($condition["key"]) && !empty($condition["valuelist"])) {
			if(!is_array($condition["key"])) return;
			if(!is_array($condition["valuelist"])) return;
			
			//验证所有的插入字段是否存在
			foreach($condition["key"] as $fieldname)
				if(!isset($this->fields[$fieldname])) func_throwException( "数据表：{$this->tableName} 中无此字段 {$fieldname}!" );
		...
		foreach($condition as $key=>$val){
			...
			if( $this->validator($this->formatKey($fieldname)) ) $list[$key]=$val;
		}
	...
	private function initRule(){
		...
		foreach($this->fields as $key=>$val) $this->rule[$key]=true;
	}
	...
	public function flushTableInfo() {
		...
		foreach ($result as $key => $val) {
			...
			if( strtolower($val['Key']) == 'pri' )$pks[] = $val['Field'];
		}
	...
	protected function getTableInfo(){
		...
		if( file_exists( $cache ) ){
			...
			$this->fields = $info;
			if(isset($this->fields[0])) $this->flushTableInfo();//自动清除老版本的表结构缓存
		}else{
			$this->flushTableInfo();
		}
	}
