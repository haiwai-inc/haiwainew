...
final class Factory {
	...
	private function doAction($ACT){
		...
		if(isset($conf['tpl'])) $ACT->assign("sitetpl",$conf['tpl']);//加载站点模板路径
	...
	private function startSession(){
		...
		if(isset($_SESSION['UserLevel'])){
			if(in_array($_SESSION['UserLevel'],array(6,7))){
				$this->redirectHttps();
			}
		}
	...
	private function redirectHttps(){
		...
		if(!$this->isHTTPS()){
			$request_uri = isset($_SERVER['HTTP_X_FORWARDED_REQUEST_URI'])?$_SERVER['HTTP_X_FORWARDED_REQUEST_URI']:$_SERVER['REQUEST_URI'];
			go("https://{$_SERVER['HTTP_HOST']}{$request_uri}");
		}
	}
