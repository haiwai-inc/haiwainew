...
function smarty_function_geoFilterCode($params, &$smarty){
	...
	//对输出的iframe code进行ip过滤
	if(strstr($code,"src=\"/service/ad/script/js.php?ip=[ipaddress]&")){
		$ipstr=empty($_GET['ip'])?"":"ip={$_GET['ip']}&";
		$code = str_replace("src=\"/service/ad/script/js.php?ip=[ipaddress]&", "src=\"/service/ad/script/js.php?{$ipstr}", $code);
	}
