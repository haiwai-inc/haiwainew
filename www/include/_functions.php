...
function func_getNodes(){
	...
	if(empty($webnodes)){
		...
		if(substr($_GlobalConfig['host'],0,2)=='10'){
			$webnodes = include DOCUROOT."/inc.webnodes.php";
		}else{
			$webnodes = include DOCUROOT.'/admin/tools/srcSync/svnServerList.php';
		}
	}
 ...
 * 检查是否是超管
 */
function func_isSuperAdmin(){
	$adminIpWhiteList = ['96.90.222.145','96.90.222.146','96.90.222.147','96.90.222.148','127.0.0.1'];
	$ip = http::getIP();
	if(in_array($ip, $adminIpWhiteList)) return true;
...
function func_getSmarty($appName,$centreModel=false){
    ...
    $smartyFolderName=(func_isAMP($appName))?"Amp":"Tpl";
    $smarty->setTemplateDir( $centreModel?DOCUROOT.'/template/'.$appName.'/':DOCUROOT.'/'.$appName.'/'.$smartyFolderName.'/' )
...
//检查当前页面是否需要使用AMP模板
function func_isAMP($appName=null){
    //测试
    if(!empty($_GET['amp'])) return true;
    if(defined("AMP")) return true;
    
    //没有AMP目录
    if(!empty($appName) && !is_dir(DOCUROOT.'/'.$appName.'/Amp/')) return false;

    //绑定msite域名
    if($_SERVER['HTTP_HOST']=='m.wenxuecity.com') return true;
    
    return false;
}

//判断是否重定向用户到msite
function func_isMsite(){
    if(func_isMobile()){
        if(empty($_COOKIE['defaultBrowser']) || $_COOKIE['defaultBrowser']=='mobile') return true;
    }
...
//判断是否为移动设备
function func_isMobile(){
    if(http::isTablet()) return false; 
 ...
 * i.g.
	...
	$config=array(
			...
			//验证信息
			'user'=>'SMTP_Injection',
			'pass'=>'ced7f9baef506a1f05ba61ca89d593325e816b84',
			
			//显示信息
			'name'=>'论坛管理',
			'fromuser'=>'notification@mail.wenxuecity.com',
	...
	$status=func_sendMail(
		'文学城系统提示邮件',
		'<h1>Hello World! 这是一个测试邮件</h1><b><a href="http://www.wenxuecity.com">访问网站</a>content</b>',
		'nothing',
...
function func_sendMail($title,$html,$text,$email,$user,$config=array()){
	...
	require_once(DOCUROOT."/include/mailer/class.pop3.php");
	require_once(DOCUROOT."/include/mailer/class.smtp.php");
...
function func_checkUserIP($maxnum=500,$host='127.0.0.1',$port=11211, $expire=86400){
	...
	if ($count > $maxnum){
		...
		if(in_array($ptr,$spiderlist)||in_array($ip,$spiderIPlist)){
			//被允许的搜索引擎
			$memcache->add($whiteIP, 1, 0);
			return false;
		}else{
			$memcache->add($blackIP, 1, 0);
			//非法用户
			return true;
		}
	}
...
//检查Cron执行进度
function func_checkCronLock($id,$argv,$action,$loop=3)
{
	unset($argv[0]);
	foreach($argv as $v)
		$id.="_".$v;
	
	$obj_cron_log=load("members_cron_log");
	
	$memObj=func_initMemcached('cache01');
	$filename=$id;
	$id=md5($id);
	$time=times::gettime();
	
	$rs_cron_log=$obj_cron_log->getOne('*',array('file_name'=>$filename));
	
	//首次生成log
	if(empty($rs_cron_log))
		$obj_cron_log->insert(array('file_name'=>$filename));
	
	if($action=='start')
	{
		$check_lock=empty($memObj->get($id))?0:$memObj->get($id);

		//已发送邮件
		if($check_lock=='500')	
		{
			//exit("Script error, Please check the Email!");
			$obj_cron_log->update(array('repeat'=>$rs_cron_log['repeat']+1,'error'=>$rs_cron_log['error']+1),array('file_name'=>$filename));
		}
			
		//重复执行loop+1
		if($check_lock<=$loop)
		{
			$memObj->set($id,$check_lock+1,259200);
			$memObj->set($id."_time",$time,259200);
			
			$obj_cron_log->update(array('repeat'=>$rs_cron_log['repeat']+1),array('file_name'=>$filename));
		}
		else 
		{
			$memObj->set($id,500,259200);
			$obj_cron_log->update(array('repeat'=>$rs_cron_log['repeat']+1,'error'=>$rs_cron_log['error']+1),array('file_name'=>$filename));
			
			//发送邮件
			/*
			$title="wxc_cronjob error notice";
			...
			func_sendMail($title,$content,$text='',$email="sida9567@gmail.com",$user="sida9567@gmail.com",$config);
			func_sendMail($title,$content,$text='',$email="weiqi228@gmail.com",$user="weiqi228@gmail.com",$config);
			*/
		}
	}
	...
	if($action=='end')
	{
		$cron_time=$memObj->get($id."_time");
		
		$last_duration=date( "Y-m-d H:i:s", $time-$cron_time);
		
		$obj_cron_log->update(array('last_duration'=>$last_duration,'repeat'=>0),array('file_name'=>$filename));
		
		$memObj->set($id,'',1);
		$memObj->set($id."_time",'',1);
	}
}
...
//检查editor权限
function func_checkEditor(){
    
    if( empty($_SESSION ['UserLevel']) ) return false;
    if($_SESSION['UserLevel']==7)
    {
        if(isset($_SESSION['manager'][1]))
            return true;
    }
...
function setNum($type,$id,$default){
	...
	if(empty($count)){
		//第一次对对象的访问
		$count = $default+1;
		$memcache->add($memcacheID, $count, 172800); //缓存时间48小时
...
function alert($val){
	...
	//记录错误信息
	$obj=load("members_api");
	if(method_exists($obj, "error_log"))$obj->error_log($val);
...
if(!function_exists('geoip_record_by_name')){
	function geoip_record_by_name($ip){
		$databaseFile = '/usr/local/share/GeoIP/GeoLite2-City.mmdb';
		//$databaseFile = '/usr/local/share/GeoIP/GeoLite2-Country.mmdb';
		...
		if(!empty($data)){
			$result = [
				...
				'country_code3'=>$data['country']['iso_code']=='US'?'USA':$data['country']['iso_code'],
				...
				'dma_code'=>$data['location']['metro_code'],
				'area_code'=>'510',
			];
		}
